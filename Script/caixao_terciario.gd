...
@export var animation_name: String = "caixao003"
...
func _ready() -> void:
	...
	if _area_ref:
		...
		push_warning("Caixao3: não encontrou AreaInteracao ancestor com sinais. Verifique hierarquia.")
	...
	if _blocker_shape:
		...
		push_warning("Caixao3: blocker (Blocker/CollisionShape2D) não encontrado. Adicione StaticBody2D->CollisionShape2D como filho chamado 'Blocker' para bloquear o player no começo.")
...
func _on_area_interacao_interacted() -> void:
	...
	if _blocker_shape:
		...
		# se não achou o blocker, ainda assim continua com a animação
		push_warning("Caixao3: não foi possível desativar colisão (blocker ausente).")
	...
	if sf and animation_name in sf.get_animation_names():
		...
		push_error("Caixao3: SpriteFrames ausente ou a animação '%s' não existe." % animation_name)
